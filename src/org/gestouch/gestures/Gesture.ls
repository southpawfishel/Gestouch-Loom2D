package org.gestouch.gestures
{
	...
	public class Gesture extends EventDispatcher
	{
		 ...
		 */
		protected var _gesturesToFail:Dictionary.<Gesture, Object> = new Dictionary.<Gesture, Object>(true);
		protected var _pendingRecognizedState:GestureState;
		
		private var eventListeners:Dictionary.<Function, Object> = new Dictionary.<Function, Object>();
		 ...
		 * 
		 */
		public function get gestouch_internal_targetAdapter():IGestureTargetAdapter
		{
		...
		public function get gestouch_internal_idle():Boolean
		{
			return _idle;
		}
...
//			eventListeners = new Dictionary(true);
		...
		public function gestouch_internal_canBePreventedByGesture(preventingGesture:Gesture):Boolean
		{
			return true;
		}
		
		
		public function gestouch_internal_canPreventGesture(preventedGesture:Gesture):Boolean
		{
			return true;
		}
		...
		protected function installTarget(target:Object):void
		{
			if (target)
			{
				_gesturesManager.gestouch_internal_addGesture(this);
			}
		}
		...
		protected function uninstallTarget(target:Object):void
		{
			if (target)
			{
				_gesturesManager.gestouch_internal_removeGesture(this);
			}
		}
		...
		protected function setState(newState:GestureState):Boolean
		{
			...
			if (newState == GestureState.BEGAN || newState == GestureState.RECOGNIZED)
			{
				...
				for (key in _gesturesToFail)
				{
					...
					if (!gestureToFail.gestouch_internal_idle &&
			...
			if (_state.gestouch_internal_isEndState)
			{
				_gesturesManager.gestouch_internal_scheduleGestureStateReset(this);
			}
			...
			if (hasEventListener(_state.gestouch_internal_toEventType()))
			{
				dispatchEvent(new GestureEvent(_state.gestouch_internal_toEventType(), _state, oldState));
			}
			...
			{
				_gesturesManager.gestouch_internal_onGestureRecognized(this);
			}
		...
		public function gestouch_internal_setState_internal(state:GestureState):void
		{
			setState(state);
		}
		...
		public function gestouch_internal_touchBeginHandler(touch:Touch):void
		{
		...
		public function gestouch_internal_touchMoveHandler(touch:Touch):void
		{
			_touchesMap[touch.id] = touch;
			onTouchMove(touch);
		}
		...
		public function gestouch_internal_touchCancelHandler(touch:Touch):void
		{
			...
			if (!state.gestouch_internal_isEndState)
			{
				if (state == GestureState.BEGAN || state == GestureState.CHANGED)
				{
					setState(GestureState.CANCELLED);
